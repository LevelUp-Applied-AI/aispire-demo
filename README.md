...
1. **Operators of production AI-employee orgs** — practical references for the orchestration choices a growing org has to make.
2. **Engineers evaluating Claude Code's 2026-native primitives vs. Software Factory / Gas City** — a side-by-side mapping of capabilities to layers.
3. **Anyone diagnosing silent-pass bugs in their own QA harness** — a deterministic reproduction with a fail-closed alternative.

## What "AI employees" means here

In `aispire`, AI agents are treated as hires rather than tools. The framing forces commitment to four things any human hire gets: a job description (system prompt), a scope (allowed tools, file paths), a quality bar (structured output protocol), and an accountability chain (lifecycle gates). Most DIY agent rigs skip two of those four.

The `aispire` org chart, sanitized for this repository, looks like this:
...
| Learning Sciences | 1 | Verifies runtime evidence supports declared learning outcomes |
| Learner Experience | 1 | Plays a pilot-cohort learner; reports issues with the assignment instructions |
| Cohort Insights | 1 | Live staff-facing chatbot synthesizing weekly learner feedback |
| External Comms | 1 | Builds external/community talks |

Models are mixed by role: 14 employees use Claude (Code or API), 3 use Gemini (Pro or 2.5-flash-image, for visual audit), 1 uses OpenAI (Responses API with the `image_generation` tool, for image generation). Vendor uniformity is not a goal; each role uses the model that fits the work.

## The orchestration question

Past several AI hires, agent orchestration becomes org design — the same problems any growing team faces: specialization, scope, handoffs, and accountability. The architectural choice is a layer decision, not a tool selection:
...
These layers stack. They don't substitute for each other. Treating them as alternatives is the bias trap; identifying the layer first and the tool second is the productive frame.
...
## The example reproduced here

Across `aispire`'s six Curriculum QA auditors, status was originally inferred from `claude --print` stdout via `grep -c '\[ERROR\]'`. A crashed auditor produced no output; the harness counted zero errors and reported a clean pass. The same pattern appears in any DIY rig that reads stdout to determine pass/fail — and is the same defect class autograding systems explicitly forbid in education ([`Unmodified Starter Failure Rule`](https://en.wikipedia.org/wiki/Software_testing#Functional_vs_non-functional) is the closest published equivalent).
...
- `runner_legacy.sh` — a faithful reproduction of the broken pattern. Ships green on a crashed agent.
